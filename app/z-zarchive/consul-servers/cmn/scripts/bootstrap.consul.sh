...
mkdir -p /etc/consul.d/consul
mkdir /var/consul
#Config Files Download
aws s3 cp s3://tf-consul/config-consul.json /etc/consul.d/consul/config.json
...
aws s3 cp s3://tf-consul/leader.txt /tmp/leader.txt
#Config Files Upload
...
unzip consul.zip
useradd consul
chown -R consul:consul /etc/consul.d/consul/config.json
chown -R consul:consul /usr/local/bin/consul
chown -R consul:consul /var/consul
#Service Config
systemctl enable consul.service
#Service Start
runuser -l consul -c '/usr/local/bin/consul agent -config-dir=/etc/consul.d/consul'
