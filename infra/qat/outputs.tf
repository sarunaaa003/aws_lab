...
output "ENV_VPC_ID" {
  value = "${terraform_remote_state.infra.output.VPC_QAT_ID}"#replace "VPC_CMN" for relevant VPC's
}
output "R53_PRIVATE_ZONE" {
  value = "${terraform_remote_state.infra.output.R53_PRIVATE_ZONE}"
}
output "VPC_PEER_CMN_QAT" {
  value = "${terraform_remote_state.infra.output.cmn_qat.id}"#replace "cmn_dev" for relevant VPC's
}
